...
export const formatHour = (item) => {
  ...
  timetable.push(
    `${item.vuelo_Ida.salida.split("T")[1].split(":")[0]}:${
      item.vuelo_Ida.salida.split("T")[1].split(":")[1]
    }`
  );
  timetable.push(
    `${item.vuelo_Ida.llegada.split("T")[1].split(":")[0]}:${
      item.vuelo_Ida.llegada.split("T")[1].split(":")[1]
    }`
  );

  if (item.vuelo_Vuelta) {
    timetable.push(
      `${item.vuelo_Vuelta.salida.split("T")[1].split(":")[0]}:${
        item.vuelo_Vuelta.salida.split("T")[1].split(":")[1]
      }`
    );
    timetable.push(
